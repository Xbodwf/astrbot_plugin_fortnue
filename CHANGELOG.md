...
### 新增
- **图源忽略列表配置**：支持在配置中设置 `ignored_sources`，使用 `^jrys` 或 `^jrysn` 时不会抽到配置的图源。
- **图源权重配置**：支持为图源配置抽取权重 (`source_weights`)，权重越高被抽中概率越大。
- **图片审查功能**：
  - 支持 AstrBot 内置提供商和 OpenAI Compatible API 两种审查方式。
  - 支持图源过滤模式（黑名单/白名单）。
  - 支持多种审核失败处理方式：重试、切换图源、提示用户、**打码**。
- **打码功能**：
  - 审查发现不适宜内容时，AI 输出坐标，自动对指定区域打马赛克。
  - 支持自定义打码审查提示词 (`mosaic_prompt`)。
  - 默认提示词针对二次元图片优化，避免正常插画误判。
- **`jrysl restore` 指令**：重置所有用户的运势数据。
- **打包脚本**：新增 `package.sh` 用于打包插件为 zip 文件。

### 修复
- 修复 RGBA 图片保存为 JPEG 的错误。
- 修复头像下载逻辑。
- 修复模块相对导入问题。
- 修复审查方法返回值数量不一致的问题。

### 重构
- **代码模块化**：将原 `main.py` 拆分为模块化结构：
  - `config/`：配置加载模块
  - `core/`：核心功能模块（图片审查、背景管理、运势生成）
  - `utils/`：工具模块（颜色处理、图片处理、HTTP 请求）

## 1.30.0

### 新增
- **忽略图源列表配置**：支持在配置中设置 `ignored_sources`，使用 `^jrys` 或 `^jrysn` 时不会抽到配置的图源，必须通过 `^jrysl source {图源名}` 显式指定才能使用。
- **图源权重配置**：支持在配置中设置 `source_weights`（格式：`"图源名:权重"`），为不同图源配置抽取权重，权重越高被抽中概率越大，未配置权重的图源默认权重为 1。
- **图片审查功能**：
  - 支持在配置中启用图片内容审查 (`image_moderation`)。
  - 支持两种审查提供商：
    - **AstrBot 内置提供商**：通过 `builtin_provider_id` 配置，支持下拉选择。
    - **OpenAI Compatible API**：通过 `openai_api_key`、`openai_api_base`、`openai_model` 配置。
  - 审查不通过时支持自动重试，可配置最大重试次数 (`max_retries`)。
...
## 1.29.0
- 指令调整：新增 `jrysn` 指令用于随机刷图（无运势信息）。
- 指令调整：将原 `jrysl none` 的随机刷图功能迁移至 `jrysn`，保留 `jrysl none source {图源名称}` 指令。
